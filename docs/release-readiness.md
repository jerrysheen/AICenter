...
当前要完成的是隐私、安全和稳定性验收，**不继续扩大功能**。不加 Intent Router、不多 Agent、不换数据库体系。Ask / 文章阅读的生产执行器是冻结 DeepSeek Harness；不在这轮设计资料搜索插件。本地循环只作测试注入或显式回退。0.2.0 发布门槛仍是隐私、安全和稳定性，不因此扩大产品功能。
