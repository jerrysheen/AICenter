...
产品入口是问答页「文章阅读」和阅读页「分析」。内部 Job 是 `ai.article.analyze`。这是 Search Agent 的当前落地，不是第二条 Agent Runtime。
...
`AiSession.kind = article-analysis`。`AiRun.taskType = article-analysis.reader.v1`。`outputText` 上限 40,000 字。
